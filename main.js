...
let OPERATIONS_HEADERS = ['id', 'type', 'amount', 'source account', 'paging token', 'price', 'asset code', 'asset type', 'asset issuer', 'buying asset code', 'buying asset type', 'buying asset issuer', 'selling asset code', 'selling asset type', 'selling asset issuer', 'offer id', 'from', 'to', 'into', 'account', 'funder', 'starting balance', 'signer key', 'signer weight', 'master key weight', 'low threshold', 'med threshold', 'high threshold', 'home domain', 'trustee', 'trustor']
function operation_get(operation){
  return [operation.id, operation.type, operation.amount, operation.source_account, operation.paging_token, operation.price, native2xlm(operation.asset_code, operation.asset_type), operation.asset_type, operation.asset_issuer, native2xlm(operation.buying_asset_code, operation.buying_asset_type), operation.buying_asset_type, operation.buying_asset_issuer, native2xlm(operation.selling_asset_code, operation.selling_asset_type), operation.selling_asset_type, operation.selling_asset_issuer, operation.offer_id, operation.from, operation.to, operation.into, operation.account, operation.funder, operation.starting_balance, operation.signer_key, operation.signer_weight, operation.master_key_weight, operation.low_threshold, operation.med_threshold, operation.high_threshold, operation.home_domain, operation.trustee, operation.trustor]
}
...
function effect_get(effect){
  return [effect.id, effect.type, effect.amount, effect.asset_type, effect.paging_token, effect.account, effect.offer_id, effect.seller, effect.bought_amount, effect.sold_amount, native2xlm(effect.bought_asset_code, effect.bought_asset_type), effect.bought_asset_type, effect.bought_asset_issuer, native2xlm(effect.sold_asset_code, effect.sold_asset_type), effect.sold_asset_type, effect.sold_asset_issuer]
}
...
function offer_get(offer){
  return [offer.id, offer.amount, offer.price, offer.seller, native2xlm(offer.buying.asset_code, offer.buying.asset_type), offer.buying.asset_type, offer.buying.asset_issuer, native2xlm(offer.selling.asset_code, offer.selling.asset_type), offer.selling.asset_type, offer.selling.asset_issuer, offer.paging_token]
}
